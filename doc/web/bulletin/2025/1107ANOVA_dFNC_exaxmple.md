...
12. Skipping the display, you may jump to the [Stats'] button and select your {prefix}_dfnc.mat file. Then you will also get the option to remove nuisance covatiates (which may be useful for multi site data), but in our case we click [No]. Then you need to select your output folder.<BR>
13. A new window pops up, where you need to change the design criteria to "1-way, x-level anova" in the dropdown and then a small "ANOVA Levels" window pops up where you enter number of levels in the textbox, perhaps 3 and then hit the [OK] button (not all ANOVA features work for more than 5 levels).<BR>
