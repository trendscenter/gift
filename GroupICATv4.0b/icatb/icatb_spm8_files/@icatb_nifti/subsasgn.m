...
case {'.'},
    ...
    for i=1:length(varargin),
        val     = varargin{i};
        obji    = class(objs(i),'icatb_nifti');
        obji    = fun(obji,subs,val);
        objs(i) = struct(obji);
    end;
    obj = class(objs,'icatb_nifti');
...
case {'.'},
    ...
    for ii=1:numel(objs)
        ...
        if length(subs)>1, % && ~strcmpi(subs(1).subs,{'raw','dat'}),
            val0 = subsref(class(obj,'icatb_nifti'),subs(1));
...
if isa(val,'icatb_file_array'),
    ...
    error('"raw" must be of class "icatb_file_array"');
end;
return;
