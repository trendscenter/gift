function V = icatb_spm_vol(P)
% Get header information etc for images.
% FORMAT V = icatb_spm_vol(P)
...
%       V.dt    - A 1x2 array.  First element is datatype (see icatb_spm_type).
...
%
% Note that icatb_spm_vol can also be applied to the filename(s) of 4-dim
...
% John Ashburner
% $Id: icatb_spm_vol.m 2237 2008-09-29 17:39:53Z guillaume $
...
[pth,nam,ext,n1] = icatb_spm_fileparts(deblank(p));
p = fullfile(pth,[nam ext]);
n = str2num(n1);
if ~icatb_spm_existfile(p),
...
switch ext,
    ...
    case {'.img','.IMG'},
        if ~icatb_spm_existfile(fullfile(pth,[nam '.hdr'])) && ~icatb_spm_existfile(fullfile(pth,[nam '.HDR'])),
...
if isempty(n),
    V = icatb_spm_vol_nifti(p);
else
    V = icatb_spm_vol_nifti(p,n);
end;
    
if isempty(n) && length(V.private.dat.dim) > 3
    V0(1) = V;
    for i = 2:V.private.dat.dim(4)
        V0(i) = icatb_spm_vol_nifti(p, i);
    end
    V = V0;
end
