% icatb_eeg_icadefs() - function to read in a set of EEGLAB system-wide (i.e. lab-wide)
%             or working directory-wide constants and preferences. Change the 
%             way these are defined in the master icatb_eeg_icadefs.m file (usually
%             in dir eeglab/functions/sigprocfunc) or make a custom copy of 
%             the icatb_eeg_icadefs.m file in a project directory. Then, calling functions 
%             that call icatb_eeg_icadefs from an EEGLAB session in that working directory 
...
if get(0, 'screendepth') <=8 % if mono or 8-bit color
    fprintf('icatb_eeg_icadefs(): Setting display parameters for mono or 8-bit color\n');
