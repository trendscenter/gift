...
%
% Usage: >> newchans = icatb_eeg_convertlocs( EEG, 'command');
%
% Input:
...
%
% Ex:  CHANSTRUCT = icatb_eeg_convertlocs( CHANSTRUCT, 'cart2topo');
...
% convertelocs -> icatb_eeg_convertlocs
%
% Revision 1.1  2002/12/24 01:24:30  arno
% Initial revision
%

function chans = icatb_eeg_convertlocs(chans, command, varargin);

if nargin < 1
   help icatb_eeg_convertlocs;
   return;
end;
...
switch command
 case 'topo2sph',
   ...
   [sph_phi sph_theta] = icatb_eeg_topo2sph( [ [ theta{indices} ]' [ radius{indices}]' ] );
   if verbose
...
case 'topo2sphbesa',
   chans = icatb_eeg_convertlocs(chans, 'topo2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2sphbesa', varargin{:}); % search for spherical coords
case 'topo2cart'
   chans = icatb_eeg_convertlocs(chans, 'topo2sph', varargin{:}); % search for spherical coords
   if verbose
       disp('Warning: spherical coordinates automatically updated');
   end;
   chans = icatb_eeg_convertlocs(chans, 'sph2cart', varargin{:}); % search for spherical coords
case 'topo2all',
   chans = icatb_eeg_convertlocs(chans, 'topo2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2sphbesa', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2cart', varargin{:}); % search for spherical coords
...
case 'sph2topo',
 ...
 sph_phi    = {chans.sph_phi};
 indices = find(~cellfun('isempty', sph_theta));
 [chan_num,angle,radius] = icatb_eeg_sph2topo([ ones(length(indices),1)  [ sph_phi{indices} ]' [ sph_theta{indices} ]' ], 1, 2); % using method 2
 for index = 1:length(indices)
...
case 'sph2sphbesa',
   ...
   sph_phi    = {chans.sph_phi};
   indices = find(~cellfun('isempty', sph_theta));
   [chan_num,angle,radius] = icatb_eeg_sph2topo([ones(length(indices),1)  [ sph_phi{indices} ]' [ sph_theta{indices} ]' ], 1, 2);
   [sph_theta_besa sph_phi_besa] = icatb_eeg_topo2sph([angle radius], 1, 1);
   for index = 1:length(indices)
      chans(indices(index)).sph_theta_besa  = sph_theta_besa(index);
      chans(indices(index)).sph_phi_besa    = sph_phi_besa(index);
   end;   
case 'sph2all',
   chans = icatb_eeg_convertlocs(chans, 'sph2topo', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2sphbesa', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2cart', varargin{:}); % search for spherical coords
case 'sphbesa2sph',
   % using polar coordinates
   ...
   [chan_num,angle,radius] = icatb_eeg_sph2topo([ones(length(indices),1)  [ sph_theta_besa{indices} ]' [ sph_phi_besa{indices} ]' ], 1, 1);
   ...
   [sph_phi sph_theta] = icatb_eeg_topo2sph([angle radius], 2);
   for index = 1:length(indices)
      chans(indices(index)).sph_theta  = sph_theta(index);
      chans(indices(index)).sph_phi    = sph_phi  (index);      
   end;
case 'sphbesa2topo',
   chans = icatb_eeg_convertlocs(chans, 'sphbesa2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2topo', varargin{:}); % search for spherical coords
case 'sphbesa2cart',
   chans = icatb_eeg_convertlocs(chans, 'sphbesa2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2cart', varargin{:}); % search for spherical coords   
case 'sphbesa2all',
   chans = icatb_eeg_convertlocs(chans, 'sphbesa2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2all', varargin{:}); % search for spherical coords
case 'cart2topo',
   chans = icatb_eeg_convertlocs(chans, 'cart2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2topo', varargin{:}); % search for spherical coords
case 'cart2sphbesa',
   chans = icatb_eeg_convertlocs(chans, 'cart2sph', varargin{:}); % search for spherical coords
   chans = icatb_eeg_convertlocs(chans, 'sph2sphbesa', varargin{:}); % search for spherical coords
