function icatb_conn_html_report(param_file, opts)
...
if (~exist('param_file', 'var'))
    filterP = ['*', PARAMETER_INFO_MAT_FILE, '*.mat'];Grou
...
resultsDir =  fullfile(outputDir, [sesInfo.userInput.prefix, '_conn_results']);
try
...
varNames = {'ComponentNumber', 'Loadings', 'SpatialMaps'};

if (saveFigInfo)
    
    if (~strcmpi(resultsFormat, 'pdf'))
        
        resultsInfo(end + 1).title = 'Kurtosis of loadings and spatial maps';
...
if (saveFigInfo)
    ...
    if (~strcmpi(resultsFormat, 'pdf'))
        ...
        htmlSummaryStr(end + 1). title = 'Kurtosis of loadings and spatial maps';
...
D(size(D,2)+1).string = ['Number Of Voxels : ', num2str(sesInfo.numOfScans)];
...
scaleType = sesInfo.scaleType;
if (isnumeric(scaleType))
    scaleType = deblank(calibrateOptions(sesInfo.scaleType + 1, :));
end
D(size(D,2)+1).string = ['Scaling Components : ', scaleType];
