% icatb_eeg_vararg2str() - transform arguments into string for evaluation 
%                using the eval() command
%
% Usage:
%   >> strout = icatb_eeg_vararg2str( allargs );
%   >> strout = icatb_eeg_vararg2str( allargs, inputnames, inputnum, nostrconv );
%
% Inputs:
...
function strout = icatb_eeg_vararg2str(allargs, inputnam, inputnum, int2str );

if nargin < 1
	help icatb_eeg_vararg2str;
	return;
end;
...
for index = 1:length(allargs)
	...
	if ~isempty(inputnam{index})
		...
		elseif iscell( tmpvar )
			strout = [ strout ',{' icatb_eeg_vararg2str( tmpvar ) '}' ];
...
function str = struct2str( structure )
	...
	for index = 1:length( allfields )
		...
		str = [ str, '''' allfields{index} ''',{' icatb_eeg_vararg2str( allcontent ) '},' ];
	end;
