% icatb_eeg_finputcheck() - check Matlab function {'key','value'} input argument pairs
%
% Usage: >> result = icatb_eeg_finputcheck( varargin, fieldlist );
%        >> [result varargin] = icatb_eeg_finputcheck( varargin, fieldlist, ... 
%                                                         callingfunc, mode );
% Input:
...
%	result = icatb_eeg_finputcheck(varargin, ...
%               { 'title'         'string'   []       ''; ...
%                 'percent'       'real'     [0 1]    1 ; ...
...
function [g, varargnew] = icatb_eeg_finputcheck( vararg, fieldlist, callfunc, mode )

	if nargin < 2
		help icatb_eeg_finputcheck;
		return;
	end;
	if nargin < 3
		callfunc = '';
...
function g = fieldtest( fieldname, fieldtype, fieldval, tmpval, callfunc );
    ...
    switch fieldtype
     ...
     case '';
     otherwise, error([ 'icatb_eeg_finputcheck error: unrecognized type ''' fieldname '''' ]);
    end;
