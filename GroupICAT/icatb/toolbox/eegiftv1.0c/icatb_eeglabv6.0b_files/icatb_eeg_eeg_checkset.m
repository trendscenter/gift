% icatb_eeg_eeg_checkset()   - check the consistency of the fields of an EEG dataset 
%                    Also: See EEG dataset structure field descriptions below.
%
% Usage: >> [EEGOUT,result] = icatb_eeg_eeg_checkset(EEG); % perform all checks
%                                                  except 'makeur'
%        >> [EEGOUT,result] = icatb_eeg_eeg_checkset(EEG, 'keyword'); % perform 'keyword' check(s)
%
% Inputs:
...
% $Log: icatb_eeg_eeg_checkset.m,v $
...
% icatb_eeg_eeg_checkset() -> icatb_eeg_eeg_checkset in warnings
%
...
function [EEG, res] = icatb_eeg_eeg_checkset( EEG, varargin );
msg = '';
res = 0; % 0 = OK, 1 = error, -1=warning
com = sprintf('%s = icatb_eeg_eeg_checkset( %s );', inputname(1), inputname(1));

if nargin < 1
    help icatb_eeg_eeg_checkset;
    return;
end;
...
if length(EEG) > 1
    ...
    for index = 1:length(EEG)
        if ~isempty(EEG(index).data)
            if ~isempty( varargin)
                [TMP, res] = icatb_eeg_eeg_checkset(EEG(index), varargin{:});
            else
                [TMP, res] = icatb_eeg_eeg_checkset(EEG(index));
            end;
            tmpsaved = TMP.saved;
...
if isempty(EEG.data)
    ...
    error('icatb_eeg_eeg_checkset error: no data'); return;
end;              

if ~isempty( varargin)
    if isempty(EEG.data)
        errordlg2('Empty dataset -> File / Import data or File / Load existing dataset', 'Error');
        error('icatb_eeg_eeg_checkset error: empty dataset'); return;
    end;    
end;
        ...
        switch varargin{ index }
         ...
         case 'chanlocs_homogeneous', 
          ...
          if ~isfield(EEG.chanlocs, 'X') | isempty(EEG.chanlocs(1).X)
              ...
              res = [ inputname(1) ' = icatb_eeg_eeg_checkset('  inputname(1) ', ''chanlocs_homogeneous'' ); ' ];
          end;
         case 'chanlocsize', 
         ...
         case 'makeur', 
          if ~isempty(EEG.event)
              if isfield(EEG.event, 'urevent'), 
                  ...
                  disp('icatb_eeg_eeg_checkset note: re-creating the original event table (EEG.urevent)');
              else
                  disp('icatb_eeg_eeg_checkset note: creating the original event table (EEG.urevent)');
              end;
         ...
         case 'eventconsistency',          
          [EEG res] = icatb_eeg_eeg_checkset(EEG);
          ...
          if isfield(EEG.event, 'latency')
              ...
              if (length(I1) + length(I2)) > 0 
                  fprintf('icatb_eeg_eeg_checkset warning: %d/%d events had out-of-bounds latencies and were removed\n', ...
          ...
          if ~isempty(EEG.event) & isfield(EEG.event, 'epoch')
              ...
              try, allepochs = [ EEG.event.epoch ];
                  ...
                  if ~isempty(removeevent)
                      disp([ 'icatb_eeg_eeg_checkset warning: ' int2str(length(removeevent)) ' event had invalid epoch numbers and were removed']);
                  end;
              catch, 
                  for indexevent = 1:length(EEG.event)
                      if isempty( EEG.event(indexevent).epoch ) | ~isnumeric(EEG.event(indexevent).epoch) ...
                          ...
                          disp([ 'icatb_eeg_eeg_checkset warning: event ' int2str(indexevent) ' has an invalid epoch number: removed']);
                      end;
                  end;
              end;
              ...
              % I PUT IT BACK, BUT IT DOES NOT ERASE NON-EMPTY VALUES
               ...
               for index = 1:length(difffield)
                   ...
                   if strcmp( difffield{index}, 'duration')
                       if any(valempt)
                           fprintf(['icatb_eeg_eeg_checkset: found empty values for field ''' difffield{index} ...
                                    ''' (filling with 0)\n']);
                       end;
          ...
          for index = 1:length(allfields)
              ...
              if strcmp(format, 'str')
                  fprintf('icatb_eeg_eeg_checkset note: value format of event field ''%s'' made uniform\n', allfields{index});
                  % get the field content
                  % ---------------------
                  for indexevent = 1:length(EEG.event)
          ...
          if isfield(EEG.event, 'latency')
              try, 
                  ...
                  disp('icatb_eeg_eeg_checkset: problem when attempting to resort event latencies.');     
              end;
          end;
         ...
         otherwise, error('icatb_eeg_eeg_checkset: unknown option');
        end;        
    end;
end;            
...
if ~isempty(EEG.setname)
    if ~isstr(EEG.setname)
        ...
        if size(EEG.setname,1) > 1
            disp('icatb_eeg_eeg_checkset warning: invalid dataset name, removed');
            EEG.setname = '';
        end;
    end;
else
...
if nargin > 1
    ...
    if strcmpi(varargin{1}, 'savedata') & option_storedisk
        error('icatb_eeg_eeg_checkset: cannot call savedata any more');
        ...
        if ~isstr(EEG.data) % not already saved
            ...
            res = sprintf('%s = icatb_eeg_eeg_checkset( %s, ''savedata'');', inputname(1), inputname(1));
        end;
    end;
end;
...
if isnumeric(EEG.data)
    if size(EEG.data,1) ~= EEG.nbchan
       disp( [ 'icatb_eeg_eeg_checkset warning: number of columns in data (' int2str(size(EEG.data,1)) ...
    ...
    if (ndims(EEG.data)) < 3 & (EEG.pnts > 1)
      if mod(size(EEG.data,2), EEG.pnts) ~= 0
           if popask( [ 'icatb_eeg_eeg_checkset error: the number of frames does not divide the number of columns in the data.'  10 ...
                          'Should EEGLAB attempt to abort operation ?' 10 '(press Cancel to fix the problem from the command line)']) 
               error('icatb_eeg_eeg_checkset error: user abort');
                %res = com;
               ...
               res = com;
               return;
               %error( 'icatb_eeg_eeg_checkset error: number of points does not divide the number of columns in data');
            end;        
      else
        if EEG.trials > 1
               disp( 'icatb_eeg_eeg_checkset note: data array made 3-D'); 
               res = com;
           end;    
    ...
    if size(EEG.data,3) ~= EEG.trials 
       disp( ['icatb_eeg_eeg_checkset warning: 3rd dimension size of data (' int2str(size(EEG.data,3)) ...
                   ') does not match the number of epochs (' int2str(EEG.trials) '), corrected' ]); 
       res = com;
       EEG.trials = size(EEG.data,3);
    end;    
    if size(EEG.data,2) ~= EEG.pnts 
       disp( [ 'icatb_eeg_eeg_checkset warning: number of columns in data (' int2str(size(EEG.data,2)) ...
    ...
    if     round(EEG.srate*(EEG.xmax-EEG.xmin)+1) ~= EEG.pnts          
       fprintf( 'icatb_eeg_eeg_checkset note: upper time limit (xmax) adjusted so (xmax-xmin)*srate+1 = number of frames\n'); 
       if EEG.srate == 0
          EEG.srate = 1;
       end;
    ...
    if ~isempty(EEG.event)
        if EEG.trials > 1 & ~isfield(EEG.event, 'epoch')
             if popask( [ 'icatb_eeg_eeg_checkset error: the event info structure does not contain an ''epoch'' field.'  ...
                ...
                res = com;
                return;
               %error('icatb_eeg_eeg_checkset error: no epoch field in event structure');
            end;
        end;
    else
        EEG.event = [];
    ...
    if ~isempty(EEG.epoch)
        ...
        if l ~= EEG.trials
             if popask( [ 'icatb_eeg_eeg_checkset error: the number of epoch indices in the epoch array/struct (' ...
                ...
                %return;
                 ...
                 res = com;
                 return;
                %error('icatb_eeg_eeg_checkset error: epoch structure size invalid');
            end;
        end;
    else
        EEG.epoch = [];
    ...
    icatb_eeg_eeglab_options; % changed from eeglaboptions 3/30/02 -sm
    if ~isempty(EEG.icasphere)
        if ~isempty(EEG.icaweights)
            if size(EEG.icaweights,2) ~= size(EEG.icasphere,1)
                  if popask( [ 'icatb_eeg_eeg_checkset error: number of columns in weights array (' int2str(size(EEG.icaweights,2)) ')' 10 ...
                   ...
                   'Should EEGLAB remove ICA information ?' 10 '(press Cancel to fix the problem from the commandline)']) 
                    ...
                    res = com;
                    return;
                    %error('icatb_eeg_eeg_checkset error: invalid weight and sphere array sizes');
                end;    
            end;
            if isnumeric(EEG.data)
                if length(EEG.icachansind) ~= size(EEG.icasphere,2)
                    if popask( [ 'icatb_eeg_eeg_checkset error: number of elements in ''icachansind'' (' int2str(length(EEG.icachansind)) ')' 10 ...
                ...
                if isempty(EEG.icaact) | (size(EEG.icaact,1) ~= size(EEG.icaweights,1)) | (size(EEG.icaact,2) ~= size(EEG.data,2))
                    if option_computeica
                        fprintf('icatb_eeg_eeg_checkset: recomputing the ICA activation matrix ...\n'); 
                        res = com;
                        % Make compatible with Matlab 7
                        EEG.icaweights = double(EEG.icaweights);
                        EEG.icawinv = double(EEG.icawinv);
                        if any(isnan(EEG.data(:)))
                            fprintf('icatb_eeg_eeg_checkset: recomputing using NaN indices [in first channel] ...\n');
            ...
            disp( [ 'icatb_eeg_eeg_checkset warning: weights matrix cannot be empty if sphere matrix is not, correcting ...' ]); 
            res = com;
            EEG.icasphere = [];
        end;
        if option_computeica
            if ~isempty(EEG.icaact) & ndims(EEG.icaact) < 3 & (EEG.trials > 1)
                disp( [ 'icatb_eeg_eeg_checkset note: independent component made 3-D' ]); 
                res = com;
                EEG.icaact = reshape(EEG.icaact, size(EEG.icaact,1), EEG.pnts, EEG.trials);        
            end;
        else 
            if ~isempty(EEG.icaact)
                fprintf('icatb_eeg_eeg_checkset: removing ICA activation matrix (as per edit options) ...\n'); 
            end;
            EEG.icaact     = [];
        end;
    else
...
if ~isempty( EEG.chanlocs )
    ...
    if ~isstruct( EEG.chanlocs)
        if exist( EEG.chanlocs ) ~= 2
            disp( [ 'icatb_eeg_eeg_checkset warning: channel file does not exist or is not in Matlab path: filename removed from EEG struct' ]); 
            EEG.chanlocs = [];
            res = com;
        else
            res = com;
            try, EEG.chanlocs = readlocs( EEG.chanlocs );
                  disp( [ 'icatb_eeg_eeg_checkset: channel file read' ]); 
            catch, EEG.chanlocs = []; end;
        end;     
    end;
    if isstruct( EEG.chanlocs)
        if length( EEG.chanlocs) ~= EEG.nbchan & length( EEG.chanlocs) ~= EEG.nbchan+1
            disp( [ 'icatb_eeg_eeg_checkset warning: number of channels different in data and channel file/struct: channel file/struct removed' ]); 
            EEG.chanlocs = [];
            res = com;
        end;
    end;
...
%    end;
%    disp('icatb_eeg_eeg_checkset note: creating backup chanlocs structure (urchanlocs)');
%end;
