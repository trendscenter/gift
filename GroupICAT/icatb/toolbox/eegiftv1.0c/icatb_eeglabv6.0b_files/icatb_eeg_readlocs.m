% icatb_eeg_readlocs() - read electrode location coordinates and other information from a file. 
...
%              (see File Formats).
% Usage:
%   >>  eloc = icatb_eeg_readlocs( filename );
%   >>  EEG.chanlocs = icatb_eeg_readlocs( filename, 'key', 'val', ... ); 
%   >>  [eloc, labels, theta, radius, indices] = ...
%                                               icatb_eeg_readlocs( filename, 'key', 'val', ... );
% Inputs:
...
%               a last column, icatb_eeg_readlocs() adjusts for this. Default labels are E1, E2, ...
...
% new version of icatb_eeg_readlocs
%


function [eloc, labels, theta, radius, indices] = icatb_eeg_readlocs( filename, varargin ); 

if nargin < 1
	help icatb_eeg_readlocs;
	return;
end;
...
% and pop_icatb_eeg_readlocs() functions. You do not need to edit these functions.
...
g = icatb_eeg_finputcheck( varargin, ...
   { 'filetype'	   'string'  {}                 '';
     ...
     'format'	   'cell'	 []					{} }, 'icatb_eeg_readlocs');
...
if isstr(filename)
   ...
   if isempty(g.filetype)
       ...
       fprintf('icatb_eeg_readlocs(): ''%s'' format assumed from file extension\n', g.filetype); 
   else 
   ...
   if ~isempty(g.filetype) & ~strcmpi(g.filetype, 'custom') ...
      ...
      if isempty(g.filetype) 
         error( ['icatb_eeg_readlocs() error: The filetype cannot be detected from the \n' ...
   ...
   elseif strcmp(g.filetype, 'elc')
       ...
       eloc = icatb_eeg_convertlocs(eloc, 'cart2all');
       eloc = rmfield(eloc, 'sph_theta'); % for the conversion below
       eloc = rmfield(eloc, 'sph_theta_besa'); % for the conversion below
   elseif strcmp(lower(g.filetype(1:end-1)), 'polhemus') | ...
       ...
       try, 
           ...
           disp('icatb_eeg_readlocs(): Could not read Polhemus coords. Trying to read BESA .elp file.');
           [eloc, labels, theta, radius, indices] = icatb_eeg_readlocs( filename, 'defaultelp', 'besa', varargin{:} );
       end;
   else      
       % importing file
       % --------------
       ...
       if size(array,2) < length(g.format)
           fprintf(['icatb_eeg_readlocs() warning: Fewer columns in the input than expected.\n' ...
                    '                    See >> help icatb_eeg_readlocs\n']);
       elseif size(array,2) > length(g.format)
           fprintf(['icatb_eeg_readlocs() warning: More columns in the input than expected.\n' ...
                    '                    See >> help icatb_eeg_readlocs\n']);
       end;
   ...
   if isfield(eloc, 'sph_theta_besa')
       ...
       try
           eloc = icatb_eeg_convertlocs(eloc, 'sphbesa2all');
           eloc = icatb_eeg_convertlocs(eloc, 'topo2all'); % problem with some EGI files (not BESA files)
       catch, disp('Warning: coordinate conversion failed'); end;
       fprintf('icatb_eeg_readlocs: BESA spherical coords. converted, now deleting BESA fields\n');   
   ...
   elseif isfield(eloc, 'sph_theta')
       try
           eloc = icatb_eeg_convertlocs(eloc, 'sph2all');  
       catch, disp('Warning: coordinate conversion failed'); end;
   elseif isfield(eloc, 'X')
       ...
       try
           eloc = icatb_eeg_convertlocs(eloc, 'topo2all');  
       catch, disp('Warning: coordinate conversion failed'); end;
   end;
   ...
   if ~isfield(eloc, 'labels')
       fprintf('icatb_eeg_readlocs(): Inserting electrode labels automatically.\n');
       for index = 1:length(eloc)
   ...
   if isfield(eloc, 'channum')
       ...
       if any( sort(allchannum) ~= allchannum )
           fprintf('icatb_eeg_readlocs(): Re-sorting channel numbers based on ''channum'' column indices\n');
    ...
    if isstruct(filename)
        ...
        disp('icatb_eeg_readlocs(): input variable must be a string or a structure');
    end;        
end;
...
function array = load_file_or_array( varname, skiplines );
    ...
    if exist( varname ) == 2
        array = icatb_eeg_loadtxt(varname,'verbose','off','skipline',skiplines);
    else % variable in the global workspace
         % --------------------------
         try, array = evalin('base', varname);
	     catch, error('icatb_eeg_readlocs(): cannot find the named file or variable, check syntax');
		 end;
    end;     
return;
...
function [str, mult] = checkformat(str)
    ...
    error(['icatb_eeg_readlocs(): undefined field ''' str '''']);
