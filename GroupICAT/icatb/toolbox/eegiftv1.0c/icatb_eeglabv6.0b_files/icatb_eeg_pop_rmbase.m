% icatb_eeg_pop_rmbase() - remove channel baseline means from an epoched or 
%                continuous EEG dataset. Calls rmbase().
% Usage:
%   >> OUTEEG = icatb_eeg_pop_rmbase( EEG ); % pop up an interactive arg entry window
%   >> OUTEEG = icatb_eeg_pop_rmbase( EEG, timerange, pointrange); % call rmbase()
%
% Graphic interface:
...
function [EEG, com] = icatb_eeg_pop_rmbase( EEG, timerange, pointrange);

com ='';
if nargin < 1
    help icatb_eeg_pop_rmbase;
    return;
end;
if isempty(EEG(1).data)
    disp('icatb_eeg_pop_rmbase(): cannot remove baseline of an empty dataset'); return;
end;    
if nargin < 1
	help icatb_eeg_pop_rmbase;
	return;
...
if nargin < 2 & EEG(1).trials > 1
	...
	result       = inputdlg2( promptstr, 'Epoch baseline removal -- icatb_eeg_pop_rmbase()', ...
                                  1,  inistr, 'icatb_eeg_pop_rmbase');
	...
	if numel(result) < 2 | ((isempty(result{1}) | strcmp(result{1},'[]') ) ...
       ...
       fprintf('icatb_eeg_pop_rmbase(): using whole epoch as baseline.\n');

       % fprintf('icatb_eeg_pop_rmbase(): baseline limits must be specified.\n');
       % return; end;
    else
...
elseif nargin < 2 & EEG(1).trials == 1
	% popup window parameters
	% -----------------------
    resp = questdlg2(strvcat('Remove mean of each data channel'), 'icatb_eeg_pop_rmbase', 'Cancel', 'Ok', 'Ok');
...
% -------------------------
if length(EEG) > 1
    [ EEG com ] = eeg_eval( 'icatb_eeg_pop_rmbase', EEG, 'warning', 'on', 'params', ...
...
if exist('pointrange') ~= 1 & ~isempty(timerange)
    if (timerange(1) < EEG.xmin*1000) & (timerange(2) > EEG.xmax*1000)
        error('icatb_eeg_pop_rmbase(): Bad time range');
    end;
...
if (min(pointrange) < 1) | (max( pointrange ) > EEG.pnts)  
   error('icatb_eeg_pop_rmbase(): Wrong point range');
end;

fprintf('icatb_eeg_pop_rmbase(): Removing baseline...\n');
%
...
if EEG.trials == 1 & ~isempty(EEG.event) ...
	...
	if 0 %~isempty(boundaries) % this is crashing
        fprintf('icatb_eeg_pop_rmbase(): finding continuous data discontinuities\n');
        ...
        for index=1:length(boundaries)-1
            ...
            EEG.data(:,tmprange) = icatb_eeg_rmbase( EEG.data(:,tmprange), length(tmprange), ...
                                               [1:length(tmprange)]);
        end;
    else
        EEG.data = icatb_eeg_rmbase( EEG.data(:,:), EEG.pnts, pointrange );    
    end;		
else 
    EEG.data = icatb_eeg_rmbase( EEG.data(:,:), EEG.pnts, pointrange );
end;
...
if ~isempty(timerange)
	com = sprintf('%s = icatb_eeg_pop_rmbase( %s, [%s]);', inputname(1), inputname(1), ...
			num2str(timerange));
else
	com = sprintf('%s = icatb_eeg_pop_rmbase( %s, [], %s);', inputname(1), inputname(1), ...
