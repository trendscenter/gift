% icatb_eeg_loadtxt() - load ascii text file into numeric or cell arrays
%
% Usage:
%   >> array = icatb_eeg_loadtxt( filename, 'key', 'val' ...);
%
% Inputs:
...
% $Log: icatb_eeg_loadtxt.m,v $
% Revision 1.13  2007/03/20 02:32:38  arno
% fixed icatb_eeg_finputcheck (Andreas)
%
...
% upgrading to use icatb_eeg_finputcheck
%
...
function array = icatb_eeg_loadtxt( filename, varargin );

if nargin < 1
	help icatb_eeg_loadtxt;
	return;
end;	
if ~isempty(varargin)
...
g = icatb_eeg_finputcheck( varargin, { 'convert'   'string'   { 'on' 'off' 'force' }   'on';
