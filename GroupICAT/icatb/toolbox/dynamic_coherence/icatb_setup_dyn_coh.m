function icatb_setup_dyn_coh(param_file)
%% Setup Dynamic Coherence
%
% Inputs:
% 1. param_file - ICA Parameter file or dynamic coherence parameter file
%

icatb_defaults;
global UI_FS;

%% Select Dynamic Coherence file
if (~exist('param_file', 'var'))
    param_file = icatb_selectEntry('title', 'Select ICA/Dynamic Coherence Parameter File', 'typeEntity', 'file', 'typeSelection', 'single', 'filter', '*ica*param*mat;*dyn_coh.mat');
    drawnow;
    if (isempty(param_file))
        error('ICA/Dynamic coherence ICA parameter file is not selected');
    end
end
...
if (exist('sesInfo', 'var'))
    ...
    outputDir = icatb_selectEntry('typeEntity', 'directory', 'title', 'Select output directory to place dynamic coherence results ...');
    drawnow;
    if (isempty(outputDir))
        error('Output directory is not selected');
    end
    cohInfo.userInput.ica_param_file = param_file;
    cohInfo.userInput.outputDir = outputDir;
    cohInfo.userInput.numOfSub = sesInfo.numOfSub;
    cohInfo.userInput.numOfSess = sesInfo.numOfSess;
    cohInfo.userInput.prefix = [sesInfo.userInput.prefix, '_dyn_coh'];
    compFiles = sesInfo.icaOutputFiles(1).ses(1).name;
    cohInfo.userInput.compFiles = icatb_rename_4d_file(icatb_fullFile('directory', inDir, 'files', compFiles));
    cohInfo.userInput.numICs = sesInfo.numComp;
    cohInfo.userInput.HInfo = sesInfo.HInfo.V(1);
elseif (exist('cohInfo', 'var'))
    load (cohInfo.userInput.ica_param_file);
    cohInfo.userInput.numOfSess = sesInfo.numOfSess;
    clear sesInfo;
    outputDir = inDir;
    cohInfo.userInput.outputDir = outputDir;
    inDir = fileparts(cohInfo.userInput.ica_param_file);
else
    error('Selected file is neither ICA parameter file nor dynamic coherence parameter file');
end
...
if (~exist(deblank(cohInfo.userInput.compFiles(1, :)), 'file'))
    unzipFiles(sesInfo, sesInfo.icaOutputFiles(1).ses(1).name, fileparts(cohInfo.userInput.ica_param_file));
end

if (~exist(icatb_parseExtn(deblank(cohInfo.userInput.compFiles(1, :))), 'file'))
    ...
    cohInfo.userInput.compFiles = icatb_rename_4d_file(icatb_fullFile('directory', inDir, 'files', compFiles));
    if (~exist(deblank(cohInfo.userInput.compFiles(1, :)), 'file'))
        unzipFiles(sesInfo, compFiles, fileparts(cohInfo.userInput.ica_param_file));
    end
end


msg = 'Opening setup dynamic coherence GUI ...';

disp(msg);

msgH = helpdlg(msg, 'Setup Dynamic Coherence ...');

drawnow;

compNetworkNameData = getCompData(cohInfo.userInput.compFiles);

compGroupNames = '';
covNames = '';

if (~isfield(cohInfo.userInput, 'comp'))
    cohInfo.userInput.comp = [];
end

try
    compGroupNames = (cellstr(char(cohInfo.userInput.comp.name)));
catch
end


if (~isfield(cohInfo.userInput, 'TR'))
    if (isfield(sesInfo, 'TR'))
        cohInfo.userInput.TR = sesInfo.TR;
    else
        cohInfo.userInput.TR = 1;
    end
end
...
InputHandle = icatb_getGraphics('Dynamic Coherence Setup Analysis', 'normal', figureTag, 'off');
set(InputHandle, 'menubar', 'none');
set(InputHandle, 'userdata', cohInfo);

defaultsmenu = uimenu('parent', InputHandle, 'label', 'Dynamic Coherence Defaults', 'tag', 'dyn_coh_defaults');
...
editH = icatb_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', num2str(cohInfo.userInput.TR), 'tag', 'tr', 'fontsize', UI_FS - 1);


try
    num_clusters = cohInfo.userInput.kmeans.num_clusters;
catch
    num_clusters = 3;
end

try
    num_runs = cohInfo.userInput.kmeans.num_runs;
catch
    num_runs = 3;
end
...
promptPos(3) = promptWidth;
textH = icatb_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Enter number of K-means clusters', 'tag', 'prompt_p_kmeans', 'fontsize', UI_FS - 1);
icatb_wrapStaticText(textH);

editPos = promptPos;
editPos(1) = editPos(1) + editPos(3) + xOffset;
editPos(3) = controlWidth;
editH = icatb_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', num2str(num_clusters), 'tag', ...
    'num_clusters', 'fontsize', UI_FS - 1);


promptPos(2) = promptPos(2) - yOffset - 0.5*promptHeight;
promptPos(3) = promptWidth;
textH = icatb_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'text', 'position', promptPos, 'string', 'Enter number of K-means runs', 'tag', 'prompt_p_kmeans_runs', 'fontsize', UI_FS - 1);
icatb_wrapStaticText(textH);

editPos = promptPos;
editPos(1) = editPos(1) + editPos(3) + xOffset;
editPos(3) = controlWidth;
editH = icatb_uicontrol('parent', InputHandle, 'units', 'normalized', 'style', 'edit', 'position', editPos, 'string', num2str(num_runs), 'tag', ...
    'num_runs', 'fontsize', UI_FS - 1);
...
%     'checkbox_mdl', 'fontsize', UI_FS - 1, 'value', cohInfo.userInput.doEstimation, 'callback', {@autoMDLCallback, InputHandle});
...
if (listH == hObject)
    ...
    val = get(listH, 'value');
    try
        networkName = cohInfo.userInput.comp(val).name;
        compVals = cohInfo.userInput.comp(val).value;
    catch
    end
end

compStr = num2str((1:cohInfo.userInput.numICs)');
...
try
    ...
    if (length(cohInfo.userInput.comp) > 0)
        chk = strmatch(lower(networkName), lower(cellstr(char(cohInfo.userInput.comp.name))), 'exact');
        if (~isempty(chk))
            ind = chk;
        end
    end
    ...
    if (~exist('ind', 'var'))
        ind = length(cohInfo.userInput.comp) + 1;
    end
    
    %% Set user selected information in figure
    cohInfo.userInput.comp(ind).name = networkName;
    cohInfo.userInput.comp(ind).value =  comps(:)';
    set(handles, 'userdata', cohInfo);
    compListH = findobj(handles, 'tag', 'comp');
    set(compListH, 'string', cellstr(char(cohInfo.userInput.comp.name)));
    delete(compFigH);
...
try
    strs = cellstr(char(cohInfo.userInput.comp.name));
    cohInfo.userInput.comp(val) = [];
    strs(val) = [];
    set(listH, 'value', 1);
    set(listH, 'string', strs);
    set(figH, 'userdata', cohInfo);
catch
end
...
cohInfo = get_info_controls(handles);
fileN = fullfile(cohInfo.userInput.outputDir, [cohInfo.userInput.prefix, '.mat']);
icatb_save(fileN, 'cohInfo');
wH = icatb_dialogBox('title', 'File Saved', 'textBody', ['Dynamic coherence information is saved in file ', fileN], 'textType', 'large');
waitfor(wH);
delete(handles);
...
if (~isfield(cohInfo.userInput, 'feature_params'))
    defaultsMenuH = findobj(handles, 'tag', 'dyn_coh_defaults');
...
try
    ...
    if (~isfield(cohInfo.userInput, 'comp') || isempty(cohInfo.userInput.comp))
    ...
    cohInfo.userInput.kmeans.num_clusters = str2num(get(findobj(handles, 'tag', 'num_clusters'), 'string'));
    cohInfo.userInput.kmeans.num_runs = str2num(get(findobj(handles, 'tag', 'num_runs'), 'string'));
    ...
    icatb_run_dynamic_coherence(cohInfo);
    
catch
    icatb_errorDialog(lasterr, 'Run Error');
    rethrow(lasterror);
end
...
function cohInfo = get_info_controls(handles)
...
try
    covInfo = cohInfo.userInput.feature_params.final.tc_covariates;
catch
end

if (~exist('covInfo', 'var'))
    covInfo.numOfDataSets = cohInfo.userInput.numOfSub*cohInfo.userInput.numOfSess;
end

default_params = icatb_dyn_coh_options('covInfo', covInfo);

if (~isfield(cohInfo.userInput, 'feature_params'))
    feature_params = default_params;
else
    feature_params = cohInfo.userInput.feature_params;
end
