...
parfor nDataSet = 1:length(dataSetsToRun)
    ...
    if (~useTemporalICA)
        ...
        if (strcmpi(backReconType, 'spatial-temporal regression'))
            ...
            if (strcmpi(backReconType, 'constrained ica'))               
                % constrained ica
                ICA_Options = icatb_icaOptions(size(data), 'constrained ica (spatial)', 'off');
                num_comps = rank(data);
                [tmpW, tmpDw] = icatb_calculate_pca(data, num_comps, 'remove_mean', 0, 'whiten', 1);
                ICA_Options = {'ref_data', icasig, ICA_Options{:}};
                [dd, ddW, tmpA, spatial_maps] = icatb_icaAlgorithm('constrained ica (spatial)', tmpW', ICA_Options);
                tc = tmpDw*tmpA;
            else
                %% GIG -ICAR
