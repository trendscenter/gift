function varargout = icatb_spm_BIDS(varargin)
...
elseif nargin == 1
    if ischar(varargin{1})
        root = varargin{1};
        %root = icatb_spm_select('CPath',varargin{1});
    else
        varargout = varargin(1);
        return;
    end
else
    BIDS = icatb_spm_BIDS(varargin{1});
...
%==========================================================================
try
    BIDS.description = icatb_spm_jsonread(fullfile(BIDS.dir,'dataset_description.json'));
catch
...
p = icatb_listFiles_inDir(BIDS.dir,'participants*.tsv');
p = icatb_fullFile('directory', BIDS.dir, 'files', p);
if ~isempty(p)
    BIDS.participants = icatb_spm_load(p);
end
%p = spm_select('FPList',BIDS.dir,'^participants\.json$');
p = icatb_listFiles_inDir(BIDS.dir,'participants*.json');
p = icatb_fullFile('directory', BIDS.dir, 'files', p);
if ~isempty(p)
    BIDS.participants.meta = icatb_spm_jsonread(p);
end
...
sub = dir(fullfile(BIDS.dir, 'sub*'));
sub = sub([sub.isdir] == 1);
sub = cellstr(char(sub.name));
if isequal(sub,{''})
...
for su=1:numel(sub)
    ...
    sess = dir(fullfile(fullfile(BIDS.dir,sub{su}), 'ses*'));
    sess = sess([sess.isdir] == 1);
    sess = cellstr(char(sess.name));
    for se=1:numel(sess)
...
if exist(pth,'dir')
    ...
    f = list_files(pth, sprintf('^%s.*_([a-zA-Z0-9]+){1}\\.nii(\\.gz)?$',subject.name));
...
if exist(pth,'dir')
    ...
    %     f = spm_select('List',pth,...
    %         sprintf('^%s.*_task-.*\\.nii(\\.gz)?$',subject.name));
    f = list_files(pth, sprintf('^%s.*_task-.*\\.nii(\\.gz)?$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
    ...
    f = list_files(pth,  sprintf('^%s.*_task-.*_events\\.tsv$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
        
        p = parse_filename(f{i}, {'sub','ses','task','acq','rec','fa','echo','inv','run','recording', 'meta'});
        subject.func = [subject.func p];
        subject.func(end).meta = icatb_spm_load(fullfile(pth,f{i})); % ?
    ...
    f = list_files(pth,  sprintf('^%s.*_task-.*_(physio|stim)\\.tsv\\.gz$',subject.name));
...
if exist(pth,'dir')
    ...
    f = list_files(pth,  sprintf('^%s.*\\.nii(\\.gz)?$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    j = 1;
    ...
    if any(~cellfun(@isempty,labels))
        ...
        for i=1:numel(idx)
            fb = icatb_spm_file(icatb_spm_file(f{idx(i)},'basename'),'basename');
            metafile = fullfile(pth,icatb_spm_file(fb,'ext','json'));
    ...
    if any(~cellfun(@isempty,labels))
        ...
        for i=1:numel(idx)
            ...
            if spm_existfile(metafile)
                subject.fmap(j).meta = {...
                    icatb_spm_jsonread(metafile),...
                    icatb_spm_jsonread(strrep(metafile,'_phase1.json','_phase2.json'))};
            else
                % (!) TODO: file can also be stored at higher levels (inheritance principle)
                subject.fmap(j).meta = struct([]); % ?
            end
...
if exist(pth,'dir')
    ...
    f = list_files(pth, sprintf('^%s.*_task-.*_eeg\\..*[^json]$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
    ...
    for i=1:numel(f)
        ...
        p = parse_filename(f{i}, {'sub','ses','task','acq','run','meta'});
        subject.eeg = [subject.eeg p];
        subject.eeg(end).meta = icatb_spm_load(fullfile(pth,f{i})); % ?
    ...
    f = list_files(pth,  sprintf('^%s.*_task-.*_channels\\.tsv$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
    ...
    f = list_files(pth,  sprintf('^%s(_ses-[a-zA-Z0-9]+)?.*_(electrodes\\.tsv|photo\\.jpg|coordsystem\\.json|headshape\\..*)$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
...
if exist(pth,'dir')
    ...
    f = list_files(pth,  sprintf('^%s.*_task-.*_meg\\..*[^json]$',subject.name));
    ...
    for i=1:numel(f)
        ...
        p = parse_filename(f{i}, {'sub','ses','task','acq','run','proc', 'meta'});
        subject.meg = [subject.meg p];
        subject.meg(end).meta = icatb_spm_load(fullfile(pth,f{i})); % ?
    ...
    f = list_files(pth,  sprintf('^%s(_ses-[a-zA-Z0-9]+)?.*_(photo\\.jpg|coordsystem\\.json|headshape\\..*)$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
...
if exist(pth,'dir')
    ...
    f = list_files(pth, sprintf('^%s.*_(events\\.tsv|beh\\.json|physio\\.tsv\\.gz|stim\\.tsv\\.gz)$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
...
if exist(pth,'dir')
    ...
    for i=1:numel(f)
        ...
        if isfield(bvalfile,'filename')
            subject.dwi(end).bval = icatb_spm_load(bvalfile.filename); % ?
        end
        ...
        if isfield(bvalfile,'filename')
            subject.dwi(end).bvec = icatb_spm_load(bvecfile.filename); % ?
        end
...
if exist(pth,'dir')
    ...
    f = list_files(pth, sprintf('^%s.*_task-.*_pet\\.nii(\\.gz)?$',subject.name));
    if isempty(f), f = {}; else f = cellstr(f); end
    for i=1:numel(f)
...
filename = icatb_spm_file(filename,'filename');
...
for n=1:N
    ...
    metafile = list_files(pth, sprintf(pattern,p.type));
    if isempty(metafile), metafile = {}; else metafile = cellstr(metafile); end
    if (~isempty(metafile))
        metafile = cellstr(icatb_fullFile('directory', pth, 'files', char(metafile)));
    end
    ...
    for i=1:numel(metafile)
        ...
        if ismeta
            if strcmp(p2.ext,'.json')
                meta = update_metadata(meta,icatb_spm_jsonread(metafile{i}));
            else
...
function files = list_files(pth, pattern)

files = icatb_listFiles_inDir(pth);
if (~isempty(files))
    files = cellstr(files);
    chk = regexp(files, pattern);
    chk = icatb_good_cells(chk);
    files = files(chk);
end