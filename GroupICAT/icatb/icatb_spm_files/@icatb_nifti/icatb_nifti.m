function h = icatb_nifti(varargin)
...
case 0
    ...
    h   = class(h,'icatb_nifti');

case 1
    if isa(varargin{1},'icatb_nifti')
        h = varargin{1};
        
    elseif ischar(varargin{1})
        if size(varargin{1},1)>1
            h = icatb_nifti(cellstr(varargin{1}));
            return;
        end
        ...
        if ~isfield(vol.hdr,'magic')
            ...
            if isfield(extras,'M') && ~isfield(extras,'mat')
                 extras.mat = extras.M;
                 if icatb_spm_flip_analyze_images
        ...
        dat   = icatb_file_array(vol.iname,dim,[dt,vol.be],offs,slope,inter);
        h     = struct('hdr',vol.hdr,'dat',dat,'extras',extras);
        h     = class(h,'icatb_nifti');

    elseif isstruct(varargin{1})
        h     = class(varargin{1},'icatb_nifti');
