function [W, cost_function, eps_yr] = icatb_tf_constrained_ivag(X, ref_data, varargin)
% Implemented into TReNDS 11/11/25 Cyrus
% Trung's implementation - 12/2022
%
% Implementation of threshold-free constrained IVA with multivariate 
% Gaussian distribution (tf-cIVA-G) that utilizes the references as 
% regularization for the IVA cost function. Both the similarity between 
% the reference and the corresponding source and the (dis)similarity 
% between that reference and the other sources are exploited. 
% tf-cIVA-G eliminates the need for constraint-threshold selection.
%
...
%
%%% Example call:
% W = tf_civag(X, ref_data, 'initW', W0, 'minIter', 100)
...
for n = 1:n_refs
    for k = 1:K
        ...
        for m = 1:n_refs
            if m ~= n
                E{n, k} = E{n, k} + dE(:, m, k) * dE(:, m, k)';
            end
        end
        % E{n, k} = E{n, k} / (N-1);
        if n <= n_refs
            E{n, k} = E{n, k} - dE(:, n, k) * dE(:, n, k)';
        end
    end
end
...
for iter = 1:Params.maxIter
    ...
    for k = 1:K
        ...
        for n = 1:n_refs
            for m = 1:n_refs
                eps_yr(m, k, n) = W(m, :, k) * dE(:, n, k);
                if m ~= n
                    cost3 = cost3 + eps_yr(m, k, n)^2;
                end
            end
            cost4 = cost4 + eps_yr(n, k, n)^2;
        end
    end
    constraint_cost = lambda / 2 * (cost3 - cost4);
    cost = cost + constraint_cost;
    ...
    for n = 1:N
        ...
        for k = 1:K
            ...
            if n <= n_refs
                dWnk = dWnk + lambda * E{n, k} * W(n, :, k)'; % Nx1
            end
    ...
    if Params.checkpoints && mod(iter, 50) == 0
        save([Params.checkpointID, 'cIVA_', num2str(iter), '_', char(datetime('now'), 'MM_dd_yyyy_HH_mm_ss'), '.mat'], 'W', ...
            'cost_function', 'eps_yr', 'stepsize', 'currentChange', 'inv_COV_N_est', 'COV_N_est', 'dWnk')
    end
end
