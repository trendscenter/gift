...
if ((step == 1) || (step == 2))
    ...
    if (~isempty(find(chkFNC == 1)))
        ...
        fncVals = icatb_compute_fnc_corr(sesInfo, tmpTR, 'filter_params', filter_cutoff, 'comps', comp_inds, 'vars_to_load', 'tc', ...
            'detrend_no', feature_params.final.fnc_tc_detrend, 'covariates', covariate_files, 'despike', feature_params.final.fnc_tc_despike, 'scansToInclude', scansToInclude, ...
            'lag', fnc_lag, 'shift_resolution', fnc_shift_resolution);
    ...
    for nF = 1:length(features)
        ...
        elseif (strcmpi(features{nF}, 'fnc correlations (lag)'))
            ...
            if (~strcmpi(desCriteria, 'mancova') && ~strcmpi(desCriteria, '1-way, x-level anova'))
                datasetNo = [mancovanInfo.ttestOpts.t.val{:}];
                fnc_corrs = fnc_corrs(datasetNo, :);
            end
