...
editTextPos(3) = editTextWidth;
editTextH = icatb_uicontrol('parent', graphicsHandle, 'units', 'normalized', 'style', 'edit', ...
    'position', editTextPos, 'String', '30', 'fontsize', UI_FS - 1, ...
...
if (~isempty(filesList))
    ...
    firstFile = icatb_listFiles_inDir(pathstr, [fileN, deblank(extn)]);
...
h_numOfPC1_cdicat = findobj(handles, 'tag', 'tag_numOfPC1_cdicat');
n_pca = str2num(get(h_numOfPC1_cdicat, 'string'));
if n_pca < 1 
