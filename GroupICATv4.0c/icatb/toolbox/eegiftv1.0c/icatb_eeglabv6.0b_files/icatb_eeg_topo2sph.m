% icatb_eeg_topo2sph() - convert a topoplot() style 2-D polar-coordinate
%              channel locations file to a 3-D spherical-angle
%              file for use with headplot()
% Usage: 
%   >> [c h] = icatb_eeg_topo2sph('eloc_file','eloc_outfile', method, unshrink);
%   >> [c h] = icatb_eeg_topo2sph( topoarray, method, unshrink );
%
% Inputs:
...
% 3-16-00 changed name to icatb_eeg_topo2sph() for compatibility with cart2topo() -sm
% 01-25-02 reformated help & license -ad 
% 03-22-02 complete remodeling for returning arguments and taking arrays -ad 

function [c, h] = icatb_eeg_topo2sph(eloc_locs,eloc_angles, method, unshrink)

MAXCHANS = 1024;

if nargin < 1
    help icatb_eeg_topo2sph;
    return;
end;
...
if isstr(eloc_locs)
	fid = fopen(eloc_locs);
	if fid<1,
	    fprintf('icatb_eeg_topo2sph()^G: cannot open eloc_loc file (%s)\n',eloc_locs)
	    return
	end
...
if nargin > 1 & isstr(eloc_angles)
	if exist(eloc_angles)==2,
	   fprintf('icatb_eeg_topo2sph: eloc_angles file (%s) already exists and will be erased.\n',eloc_angles);
	end

	fid = fopen(eloc_angles,'a');
	if fid<1,
	    fprintf('icatb_eeg_topo2sph()^G: cannot open eloc_angles file (%s)\n',eloc_angles)
	    return
	end
end;
