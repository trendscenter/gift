...
importdesignH = uimenu('parent', InputHandle, 'label', 'Import Design', 'tag', 'dfnc_design');
set(importdesignH, 'callback', {@designCallback, InputHandle});
...
function selectedRegressors = selectRegressors(spm_files)
% Select regressors
%
...
paramFile = dfncInfo.userInput.ica_param_file;
load(paramFile);
...
dispParameters.paramFile = paramFile;
dispParameters.flagTimePoints = sesInfo.flagTimePoints;
figureData.dispParameters = dispParameters;
set(hObject, 'userdata', figureData);

if (isempty(dispParameters.spmMatrices) || isempty(dispParameters.spmMatrices(1).name))
    icatb_selectDesignMatrix(hObject);
    load(paramFile);
end

spm_files = cellstr(char(sesInfo.userInput.designMatrix.name));
selectedRegressors = selectRegressors(spm_files);
dfncInfo.userInput.selectedRegressors = selectedRegressors;
set(handles, 'userdata', dfncInfo);
