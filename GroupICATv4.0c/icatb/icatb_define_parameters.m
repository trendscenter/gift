...
if (~strcmpi(modalityType, 'eeg'))
    ...
    inputText(numParameters).answerString =  char('Default Mask', 'Default + ICV Mask', 'Average Mask', 'Select Mask');
    inputText(numParameters).uiType = 'popup';
    inputText(numParameters).dataType = 'string';
    ...
    if (strcmpi(modalityType, 'fmri'))
        inputText(numParameters).help = struct('title', 'Mask', 'string', char('1) Default Mask - Mask is calculated using all the files for subjects and sessions or only the first file for each subject and session depending upon the variable DEFAULT_MASK_OPTION value in defaults. Boolean AND operation is done to include the voxels that surpass the mean of each subject''s session.', ...
            '2) Default + ICV Mask - This is same processing as Default Mask, with additional masking using SPMs intracranial volume mask (mask_ICV.nii) to remove eye artifacts', ...
            '3) Average Mask - An average mask is generated and subjects below a certain correlation threshold are excluded from the analysis. At the end of the mask generation, a GIFT batch file is created which can be used to run the group ICA.', ...
            '4) Select Mask - Masks must be in Analyze or Nifti format.'));
    else
