function varargout = post_process_sdh(varargin)
% POST_PROCESS_SDH MATLAB code for post_process_sdh.fig
%      POST_PROCESS_SDH, by itself, creates a new POST_PROCESS_SDH or raises the existing
%      singleton*.
%
%      H = POST_PROCESS_SDH returns the handle to a new POST_PROCESS_SDH or the handle to
%      the existing singleton*.
%
%      POST_PROCESS_SDH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in POST_PROCESS_SDH.M with the given input arguments.
%
%      POST_PROCESS_SDH('Property','Value',...) creates a new POST_PROCESS_SDH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before post_process_sdh_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to post_process_sdh_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help post_process_sdh

% Last Modified by GUIDE v2.5 09-Mar-2020 16:23:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @post_process_sdh_OpeningFcn, ...
    'gui_OutputFcn',  @post_process_sdh_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before post_process_sdh is made visible.
function post_process_sdh_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to post_process_sdh (see VARARGIN)

% Choose default command line output for post_process_sdh
...
% UIWAIT makes post_process_sdh wait for user response (see UIRESUME)
% uiwait(handles.post_process_sdh);


% --- Outputs from this function are returned to the command line.
function varargout = post_process_sdh_OutputFcn(hObject, eventdata, handles)
...
results.num_comps_ica = str2num(deblank(get(handles.est_clusters, 'string')));
...
% Hints: get(hObject,'String') returns contents of est_clusters as text
%        str2double(get(hObject,'String')) returns contents of est_clusters as a double
...
set(handles.est_clusters, 'string', num2str(dfncInfo.num_comps_ica));
