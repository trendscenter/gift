function [W,rho] = icatb_adaptive_constrained_ICA_EBM(X,guess_mat,varargin)
% Input:
%   X - data observations 
...
options=struct( ...
   ...
   'maxIter',512, ... % max number of iterations
   'WDiffStop',1e-6, ... % stopping criterion
   'rho_list', 0.001:0.1:0.9, ... % correlation threshold for constrained algorithm (default 0.7)
   'alpha0',1.0, ... % initial step size scaling (will be doubled for complex-valued)
   'Save_W',false ...
   );
...
for iter = 1 : max_iter_nonorth
    ...
    rho(:,iter) = tune_rho(W,X,options.rho_list,guess_mat);
    % rho - tuned constrained parameter at each iteration (num_guess x K x iter)
    % matrix where num_guess is number of constraints, iter is number of
    % iteration algorithm runs
...
%              end
             mu_old(n) = mu_c;
             mu_new(n)=max(0,mu_c + gam*(rho(n,iter)-dis_wr));
...
function [rho] = tune_rho(W,X,rho_list,guess_mat)
