function V = icatb_spm_create_vol(V)
...
if ~isfield(V,'dt') || isempty(V.dt)
    V.dt = [icatb_spm_type('float64') icatb_spm_platform('bigend')];
end
dt{1} = icatb_spm_type(V.dt(1));
...
ext = icatb_spm_file(V.fname,'ext');
switch ext
case {'img'}
    minoff = 0;
case {'nii'}
...
bits   = icatb_spm_type(V.dt(1),'bits');
...
%--------------------------------------------------------------------------
dim    = [V.dim(1:3) V.n];
dat    = icatb_file_array(V.fname,dim,[dt{1} '-' dt{2}],0,V.pinfo(1),V.pinfo(2));
N      = icatb_nifti;
N.dat  = dat;
N.mat  = V.mat;
N.mat0 = V.mat;
...
if isfield(N.extras,'mat')
    ...
    if sum(N.extras.mat(:).^2) < 1e-8*size(N.extras.mat,3)
        ...
        if ~isempty(N0) && ~isempty(N0.extras) && isstruct(N0.extras) && isfield(N0.extras,'mat')
            ...
            icatb_spm_unlink(icatb_spm_file(N.dat.fname,'ext','mat'));
        end
    end
end
