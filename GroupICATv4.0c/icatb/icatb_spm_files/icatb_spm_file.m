function str = icatb_spm_file(str,varargin)
...
if numel(options) == 1
    for n=1:numel(str)
        [pth,nam,ext,num] = icatb_spm_fileparts(deblank(str{n}));
        switch lower(options{1})
            ...
            case 'number'
                str{n} = num;
            case 'cpath'
                str{n} = icatb_spm_select('CPath',str{n});
            case 'fpath'
                str{n} = icatb_spm_fileparts(icatb_spm_select('CPath',str{n}));
            case 'unique'
                i = 1;
                while true
                    str{n} = fullfile(pth,sprintf('%s_%03d%s',nam,i,ext));
                    if ~icatb_spm_existfile(str{n}), break; else i = i + 1; end
                end
                str{n} = [str{n} num];
            case 'uniquedir'
                i = 1;
