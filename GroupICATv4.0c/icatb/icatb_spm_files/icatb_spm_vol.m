...
elseif iscell(P)
    V = cellfun(@icatb_spm_vol,P, 'UniformOutput',false);
    
else
    V = icatb_spm_vol;
    cnt = 0;
    for i=1:size(P,1)
        v = icatb_spm_vol_hdr(deblank(P(i,:)));
...
%==========================================================================
function V = icatb_spm_vol_hdr(p)
[pth,nam,ext,n] = icatb_spm_fileparts(p);
...
switch ext
    ...
    case {'.img','.IMG'}
        if ~icatb_spm_existfile(fullfile(pth,[nam '.hdr'])) && ...
           ~icatb_spm_existfile(fullfile(pth,[nam '.HDR']))
    ...
    case {'.gz','.GZ'}
        ...
        try
            ...
            V = icatb_spm_vol_hdr([tmpname n]);
            for i=1:numel(V)
                V(i).dat   = icatb_spm_read_vols(V(i));
                V(i).private.dat.fname = icatb_spm_file(p,'ext','');
                V(i).fname = p;
                V(i).dt(1) = 64;
                V(i).pinfo = [1 0 0]';
            end
...
if isempty(n) && length(V.private.dat.dim) > 3
    ...
    for i = 2:V.private.dat.dim(4)
        V0(i) = icatb_spm_vol_nifti(V.private, i);
    end
    V = V0;
end
