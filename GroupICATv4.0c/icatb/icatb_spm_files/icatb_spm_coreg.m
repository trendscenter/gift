function x = icatb_spm_coreg(varargin)
...
H = icatb_spm_hist2(VG.uint8,VF.uint8, VF.mat\icatb_spm_matrix(x(:)')*VG.mat ,sg);

% Smooth the histogram
lim  = ceil(2*fwhm);
krn1 = icatb_spm_smoothkern(fwhm(1),-lim(1):lim(1)) ; krn1 = krn1/sum(krn1); H = conv2(H,krn1);
krn2 = icatb_spm_smoothkern(fwhm(2),-lim(2):lim(2))'; krn2 = krn2/sum(krn2); H = conv2(H,krn2);
...
lim = ceil(2*fwhm);
x  = -lim(1):lim(1); x = icatb_spm_smoothkern(fwhm(1),x); x  = x/sum(x);
y  = -lim(2):lim(2); y = icatb_spm_smoothkern(fwhm(2),y); y  = y/sum(y);
z  = -lim(3):lim(3); z = icatb_spm_smoothkern(fwhm(3),z); z  = z/sum(z);
i  = (length(x) - 1)/2;
j  = (length(y) - 1)/2;
k  = (length(z) - 1)/2;
icatb_spm_conv_vol(V.uint8,V.uint8,x,y,z,-[i j k]);
