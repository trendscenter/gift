...
try
    kmeans_start = dfncInfo.kmeans_start;
catch
end

handles.kmeans_start = kmeans_start;
...
inputText(numParameters).promptString = 'Select K-means initialization';
inputText(numParameters).uiType = 'popup';
inputText(numParameters).answerString = kmeans_start_opts;
inputText(numParameters).dataType = 'string';
inputText(numParameters).tag = 'kmeans_start';
...
if (~isempty(answers))
    ...
    handles.kmeans_num_replicates = answers{4};
    try
        num_tests_est_clusters  = answers{5};
    catch
        num_tests_est_clusters = 10;
    end
    try
